...
public class RecordFileWriter {
	...
	/**
	 * Save recorded sound data into a .wav file format. Save the new .wav file in
	 * the sound file queue.
	 * 
	...
	public void save(final ByteArrayOutputStream recordBytes, final AudioFormat format) {
		Thread savingThread = new Thread() {
			public void run() {
				...
				try {
					System.out.println("Save new wav file '" + newWavFile.getFileAbsolutePath() + "'");
					...
					if (soundFileQueue != null) {
						// TODO use a non blocking method and buffer all the file name temporary (see
