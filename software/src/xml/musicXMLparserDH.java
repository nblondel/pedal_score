...
public class musicXMLparserDH {
  ...
  public musicXMLparserDH(String filename) throws IOException {
    ...
    if (doc.getElementsByTag("note").isEmpty()) {
      ...
      if (doc.getElementsByTag("note").isEmpty()) {
        System.out
            .println("Please check that your file is encoded in UTF-8 or UTF-16 and contains notes.");
      }
    }
  ...
  public String[] parseMusicXML() {
    ...
    ArrayList<Integer> voicesIndex = new ArrayList<Integer>(); // list of unique
                                                               // voices
    ...
    for (Note inote : notesOfSong) {
      ...
      if (inote.getPitch() != "Z") {
        for (int i = 0; i < inote.getDuration(); i++) {
          ...
          flatSong[i + inote.getStartTime()] += inote.getPitch()
              + inote.getAccidental();
        }
      }
    }
    ...
    for (Element thisPart : this.doc.select("part")) {
      ...
      for (Element thismeasure : thisPart.getElementsByTag("measure")) {
        ...
        if (!thismeasure.getElementsByTag("divisions").isEmpty()) {
          divisions = Integer.valueOf(thismeasure.getElementsByTag("divisions")
              .text());
        }
        ...
        for (Element thisnote : thismeasure.children()) {
          ...
          if (thisnote.tagName().equals("note")) {
            ...
            // get current voice
            currentVoice = Integer.valueOf(thisnote.getElementsByTag("voice")
                .text());
            ...
            if (!thisnote.getElementsByTag("pitch").isEmpty()) {
              for (Element thispitch : thisnote.getElementsByTag("pitch")) {
                ...
                String alter = String.valueOf(thispitch.getElementsByTag(
                    "alter").text());
              ...
              // getting other elements like rests is important for determining
              // start onsets of the next note
            ...
            if (thisnote.getElementsByTag("duration").text().isEmpty()) {
              ...
              duration = Integer.valueOf(thisnote.getElementsByTag("duration")
                  .text()) * divMultiplier.get(divisions);
